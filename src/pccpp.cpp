// PolarCodeCPPStdClass.cpp : This file contains the 'main' function. Program execution
...
void example_run_SCL() {
    unsigned nL = 4;
    int M = 72;            // code word length
    double rate = 1.0 / 2; // code rate

    unsigned K = ceil(M * rate); // information length

    // crc generator
    vector<bool> crc_g = {1, 1, 0, 0, 0, 0, 1, 1, 0, 0, 1, 0, 0,
    ...
    for (unsigned i = 0; i < N0.size(); i++) {
        // print progress
        string str = format("\nNow running EsN0: %.2f dB [%d of %d]", EsN0_dB[i], i + 1,
                            N0.size());
        cout << str << endl;
        ...
        while ((n_blks_done < n_max_blks) && (n_blk_errs[i] < 100)) {
            ...
            if (n_blks_done % 10 == 0) {
                str = format("Elapsed time: %.1f seconds, # tx blocks: %d,# error "
                             "blocks:%d, ber: %.5f, bler %.5f",
                             (clock() - tStart) / (double)CLOCKS_PER_SEC, n_blks_done,
        ...
        str = format("Elapsed time: %.1f seconds, # tx blocks: %d,# error blocks:%d, "
                     "ber: %.5f, bler %.5f",
                     (clock() - tStart) / (double)CLOCKS_PER_SEC, n_blks_done,
                     n_blk_errs[i], ber[i], bler[i]);
    ...
    cout << "Modulation:"
         << "BPSK" << endl;
    cout << "[A, CRC, K, M, R] = [" << A << ", " << crc_len << ", " << K << ", " << M
         << ", " << rate << "]" << endl;
    cout << "EsN0_dB = [";
    for (auto e : EsN0_dB)
        cout << e << " ";
